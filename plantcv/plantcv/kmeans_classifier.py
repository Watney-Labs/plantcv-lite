...
def predict_kmeans(img, model_path="./kmeansout.fit", patch_size=10):
    ...
    if len(train_img.shape) == 2:  # gray
        train_img = np.pad(
            train_img, pad_width=((before, after), (before, after)), mode="edge"
        )
    elif len(train_img.shape) == 3 and train_img.shape[2] == 3:  # rgb
        train_img = np.pad(
            train_img, pad_width=((before, after), (before, after), (0, 0)), mode="edge"
        )
    ...
    labeled = train_labels.reshape(
        reshape_params[patch_size % 2][0], reshape_params[patch_size % 2][1]
    )
    labeled = labeled.astype("uint8")
    _debug(
        visual=labeled, filename=os.path.join(params.debug_outdir, "_labeled_img.png")
    )
    return labeled
...
def mask_kmeans(labeled_img, k, cat_list=None):
        ...
        for i in L:
            ...
            _debug(
                ...
                filename=os.path.join(
                    params.debug_outdir, "_kmeans_mask_" + str(i) + ".png"
                ),
            )
    ...
    for idx, i in enumerate(cat_list):
        if idx == 0:
            ...
            mask_light = pcv.logical_or(
                mask_light, np.where(labeled_img == i, 255, 0).astype("uint8")
            )
    params.debug = debug
    _debug(
        visual=mask_light,
        filename=os.path.join(params.debug_outdir, "_kmeans_combined_mask.png"),
    )
    return mask_light
